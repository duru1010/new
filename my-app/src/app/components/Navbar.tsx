...
const Navbar = () => {
      ...
      <nav className={`fixed w-full z-[100] transition-all duration-500 ${isScrolled ? 'top-4 px-4' : 'top-0 px-0'}`}>
        <div className="max-w-7xl mx-auto">
          <motion.div className={`relative flex items-center justify-between py-3 px-6 transition-all duration-500 ${
            isScrolled ? 'rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-white/10 shadow-xl' 
            : 'bg-transparent border-b border-slate-200 dark:border-white/5'}`}>
            
            {/* Logo */}
            <div className="flex items-center gap-3">
  {/* Logo Container */}
  <div className="bg-gradient-to-tr from-blue-700 to-cyan-400 p-1.5 rounded-xl text-white shadow-lg flex items-center justify-center overflow-hidden">
  ...
  <div className="flex flex-col leading-none">
    ...
    <span className="text-[8px] font-bold text-blue-500 tracking-[0.4em] uppercase">Computers</span>
  </div>
</div>
            ...
            <div className="hidden md:flex items-center space-x-1">
              {navLinks.map((link) => (
                <a key={link.name} href={link.href} className="px-4 py-2 text-sm font-bold dark:text-slate-300 text-slate-600 hover:text-blue-600 transition-colors">
                  {link.name}
                </a>
              ))}
              ...
              <div className="relative group">
                <button className="flex items-center gap-1 px-4 py-2 text-sm font-bold dark:text-slate-300 text-slate-600 hover:text-blue-600 transition-colors">
            ...
            <div className="flex items-center gap-4">
              <button onClick={toggleTheme} className="relative w-12 h-7 bg-slate-200 dark:bg-slate-800 rounded-full p-1 transition-colors hidden sm:block">
                <motion.div animate={{ x: isDarkMode ? 20 : 0 }} className="w-5 h-5 bg-white dark:bg-blue-500 rounded-full flex items-center justify-center shadow-md">
              ...
              <button 
                ...
                className="hidden md:block px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-full text-sm font-bold transition-all shadow-lg active:scale-95"
              >
                Get Started
              </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-[150] md:hidden bg-white dark:bg-slate-900 flex flex-col"
          >
            <div className="flex items-center justify-between p-6 border-b dark:border-white/5">
              <span className="text-xl font-black dark:text-white">SOFTLINK</span>
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden space-y-2">
                      {services.map((s) => (
                        <a key={s.name} href={s.href} onClick={() => setIsOpen(false)} className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-white/5 text-lg dark:text-slate-300">
                          <div className="text-blue-500">{s.icon}</div> {s.name}
                        </a>
                      ))}
                    </motion.div>
                  )}
            ...
            <div className="p-6 space-y-4 border-t dark:border-white/5">
              <div className="flex items-center justify-between">
                ...
                <button onClick={toggleTheme} className="flex items-center gap-2 p-2 px-4 rounded-full bg-slate-100 dark:bg-white/10 dark:text-white">
                  {isDarkMode ? <Moon size={18} /> : <Sun size={18} />} {isDarkMode ? 'Dark' : 'Light'}
                </button>
              </div>
              <button onClick={() => { setShowForm(true); setIsOpen(false); }} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black text-lg">
                Get Started
              </button>
            </div>
          </motion.div>
      ...
      {/* Form Modal (Previous Implementation remains the same) */}
      <AnimatePresence>
        {showForm && (
           ...
           <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl border border-slate-200 dark:border-white/10">
             ...
             <h3 className="text-2xl font-black dark:text-white mb-6">Start Your Project</h3>
             <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
               <div className="relative"><User className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Full Name" className="w-full pl-12 pr-4 py-3 rounded-2xl bg-slate-50 dark:bg-white/5 border dark:border-white/10 dark:text-white outline-none focus:ring-2 ring-blue-500" /></div>
               ...
               <div className="relative"><MessageSquare className="absolute left-4 top-4 text-slate-400" size={18} /><textarea rows={3} placeholder="Message" className="w-full pl-12 pr-4 py-3 rounded-2xl bg-slate-50 dark:bg-white/5 border dark:border-white/10 dark:text-white outline-none focus:ring-2 ring-blue-500" /></div>
               <button className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2">Send Request <Send size={18} /></button>
             </form>
           </motion.div>
         </div>
        )}
